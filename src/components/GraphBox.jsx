...
const GraphBox = ({ selectedMlId }) => {
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/chat', {
        user_id: 'enjang69',
        ml_id: selectedMlId || '20240524-roundtrip-01',
        prompt: chatInput
      });
  ...
  return (
    <div className="flex-grow w-full h-full p-4 relative flex">
      <div className="flex-grow w-full h-full border border-gray-300 shadow-lg bg-white relative">
        {loading ? (
          <div className="flex w-grow justify-center items-center h-full">
